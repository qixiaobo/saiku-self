...
var pvc = function(def, pv, cdo) {
    ...
    pvc.text = {
        ...
        trimToWidth: function(len, text, font, trimTerminator, before, clipLen) {
            ...
            len -= Math.max(0, pv.Text.measureWidth(trimTerminator, font));
            for (;textLen > len; ) {
    ...
    def.type("pvc.visual.Scene").init(function(parent, keyArgs) {
        ...
        variable: function(name, impl) {
            var methods;
            this._vars || (this._vars = def.create(this.baseType && this.baseType._vars));
    ...
    def.type("pvc.visual.Role").init(function(name, keyArgs) {
        ...
        if (!requireIsDiscrete && def.get(keyArgs, "isMeasure")) {
            this.isMeasure = !0;
            def.get(keyArgs, "isPercent") && (this.isPercent = !0);
        }
        ...
        methods: {
            ...
            _addPropTooltip: function(ka) {
                if (!this.pvMark.tooltipOptions) {
                    ...
                    if (tooltipFormatter) {
                        var tipsyEvent = def.get(ka, "tipsyEvent") || ("near" === pointingOptions.mode ? "point" : "mouseover");
    ...
    def.type("pvc.AxisPanel", pvc.BasePanel).init(function(chart, parent, axis, options) {
        ...
        _calcLayout: function(layoutInfo) {
            ...
            if (scale.isNull) layoutInfo.axisSize = 0; else {
                var a_length = this.anchorLength(), clientSize = layoutInfo.clientSize, rangeInfo = this.axis.getScaleRangeInfo();
        ...
        _calcOverflowPaddings: function() {
            this._layoutInfo.canChange ? this._calcOverflowPaddingsFromLabelBBox() : def.debug >= 2 && this.log.warn("Layout cannot change. Skipping calculation of overflow paddings.");
        },
    ...
    def.type("pvc.NormalizedBarPanel", pvc.BarAbstractPanel).init(function(chart, parent, plot, options) {
        this.base(chart, parent, plot, options);
        chart.barChartPanel || (chart.barChartPanel = this);
    }).add({
        plotType: "nbar",
        _barVerticalMode: function() {
            return "expand";
    ...
    def.type("pvc.PointPanel", pvc.CategoricalAbstractPanel).init(function(chart, parent, plot, options) {
        ...
        _createCore: function() {
            ...
            var areaFillColorAlpha = areasVisible && linesVisible && !isStacked ? .5 : null;
            ...
            this.pvArea = new pvc.visual.Area(this, this.pvScatterPanel, {
                ...
                var color = this.base(scene, type);
                return !this._finished && color && null != areaFillColorAlpha ? color.alpha(areaFillColorAlpha) : color;
            ...
            this.pvDot = new pvc.visual.Dot(this, this.pvLine, {
                ...
                if (!dotsVisible) {
                    ...
                    if (visible && !scene.isActive) {
                        var lineWidth = Math.max(me.pvLine.lineWidth(), .2) / 2;
                        return def.sqr(lineWidth);
                    }
                }
                var v = this.base(scene);
                return scene.isInterpolated ? .8 * v : v;
            }).pvMark;
    ...
    pvc.parseSunburstColorMode = pvc.makeEnumParser("colorMode", [ "fan", "slice" ], "slice");
